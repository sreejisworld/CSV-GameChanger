...
# ===================================================================
    ...
    page_header(
        "Project Navigator",
        "Enterprise GAMP 5 project tree — live-wired to "
        "the EVOLV API with HITL compliance, Impact "
    ...
    with _p16c2:
        if st.button(
            "🔄 Reload",
            use_container_width=True,
            help="Reload the embedded navigator",
        ):
            st.rerun()
    with _p16c3:
        st.link_button(
    ...
    # ── Feature legend ─────────────────────────────────────
    st.markdown("---")
    _p16l1, _p16l2, _p16l3, _p16l4 = st.columns(4)
    with _p16l1:
        st.caption(
            "🔍 **Cmd+K** — Impact Search "
            "across all requirements, risks, test scripts"
        )
    with _p16l2:
        st.caption(
            "➕ **New Release** — Instantly creates "
            "a release with 7 GAMP 5 folders"
        )
    with _p16l3:
        st.caption(
            "🤖 **Badge** — AI-generated item "
            "awaiting human approval (FDA AI §3.2)"
        )
    with _p16l4:
        st.caption(
            "🔥 **Heatmap** — Colours folders "
            "by aggregate impact heat score (0–100)"
        )
