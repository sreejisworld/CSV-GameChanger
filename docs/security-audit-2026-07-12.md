...
| SEC-9 | **Medium** | `Agents/integrity_manager.py` | Audit rows carry per-row SHA-256 (tamper-evident per record) but each hash does not include the previous row's hash — silent row deletion/reorder would not break a chain. File is append-only by code, not by construction. | **REPORTED** — implement hash chaining (each row hashes `prev_hash`) in a future sprint; migration needs a genesis row + re-verification tooling |
| SEC-10 | **Info** | dependencies | fastapi/uvicorn/pydantic versions not pinned to CVE-audited releases; no `pip-audit` in CI | **REPORTED** — add `pip-audit` to CI and pin ranges |
