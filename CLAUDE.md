...
| URS-50.3 | Signed self-validation package PDF — 12-page document (Validation Plan · IQ · OQ with live eval results · landscape Requirements Traceability Matrix · 21 CFR Part 11 signature page), latin-1-sanitised for RTM text drawn from the living index. SELF_VALIDATION_* audit triplet. The artifact a customer's QA files before deploying EVOLV. | `utils/pdf_generator.py` (`generate_self_validation_pdf()`, `_latin1_safe()`), `API/routers/versions.py` (`POST /versions/self-validation`) |
